...
export default function TeacherGradingPage() {
  ...
  const [examTypeFilter, setExamTypeFilter] = useState<'ALL' | 'LESSON_QUIZ' | 'MIDTERM' | 'FINAL'>('ALL');
  ...
  const filteredExamResults = useMemo(() => {
    ...
    return examResults.filter((result) => {
      const matchesType = examTypeFilter === 'ALL' || result.examConfig?.examType === examTypeFilter;
  ...
  }, [examResults, examTypeFilter, query]);
  const examPageCount = Math.max(1, Math.ceil(filteredExamResults.length / examPageSize));
  const visibleExamResults = filteredExamResults.slice((examPage - 1) * examPageSize, examPage * examPageSize);
        ...
        <div className="space-y-3">
          ...
          <div className="mt-6 flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <span className="text-xs text-slate-400">Hệ thống tự chấm · hiển thị {examPageSize} kết quả mỗi trang</span>
            </div>
            <select
              value={examTypeFilter}
              onChange={(event) => setExamTypeFilter(event.target.value as typeof examTypeFilter)}
              className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 outline-none focus:border-primary"
              aria-label="Lọc loại bài thi"
            >
              <option value="ALL">Tất cả loại bài thi</option>
              <option value="LESSON_QUIZ">Quiz buổi học</option>
          ...
          {filteredExamResults.length > examPageSize && (
            <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2">
              <span className="text-xs text-slate-500">Trang {examPage}/{examPageCount}</span>
              <div className="flex items-center gap-1">
                <button
                  type="button"
                  onClick={() => setExamPage((current) => Math.max(1, current - 1))}
                  disabled={examPage === 1}
                  className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <ChevronLeft className="h-3.5 w-3.5" /> Trước
                </button>
                <button
                  type="button"
                  onClick={() => setExamPage((current) => Math.min(examPageCount, current + 1))}
                  disabled={examPage === examPageCount}
                  className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40"
                >
          ...
          {selectedSub ? (
            <>
              <div className="bg-white rounded-xl border border-slate-200 p-5">
                ...
                {selectedSub.files && (
                  <div className="mt-4">
                    <FileAttachmentViewer files={selectedSub.files} title="File bài làm của học sinh" />
                  </div>
                )}
              </div>
