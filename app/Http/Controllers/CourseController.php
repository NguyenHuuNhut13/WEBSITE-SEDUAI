...
{
    // Dữ liệu mock khóa học dùng chung
    private function getMockCourses()
    {
        return [
            [
                ...
                'benefits' => [
                    'Làm chủ Laravel Framework 11 và React 18 mới nhất.',
            ...
            [
                'title' => 'Kỹ Năng Thuyết Trình & Làm Việc Nhóm Thế Thế Hệ Mới',
    ...
    {
        $allCourses = $this->getMockCourses();
        // Lấy ra 4 khóa học tiêu biểu để hiển thị ở trang chủ
        $featuredCourses = array_slice($allCourses, 0, 4);
    ...
    public function index(Request $request)
    {
        ...
        $categories = ['Tất cả', 'Tiếng Anh', 'Lập trình', 'Kỹ năng', 'AI & Công nghệ'];

        $selectedCategory = $request->query('category', 'Tất cả');
    ...
    public function show($slug)
    {
        ...
        // Lấy 3 khóa học liên quan khác danh mục hoặc khác khóa hiện tại
        $relatedCourses = array_filter($courses, function ($c) use ($slug) {
            return $c['slug'] !== $slug;
        });
